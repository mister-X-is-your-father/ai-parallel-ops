...
tmuxで複数ペインにClaudeを配置し、完了・エラーを背景色で視覚的に検知する。管制官のレーダースコープに相当する。

### シェル関数: Claudeの完了を背景色で通知

`~/.bashrc` または `~/.zshrc` に追加:

```bash
claude-task() {
    tmux select-pane -P 'bg=colour33'   # 青 = 実行中
    claude "$@"
    if [ $? -eq 0 ]; then
        tmux select-pane -P 'bg=colour28'   # 緑 = 正常完了
    else
        tmux select-pane -P 'bg=colour160'  # 赤 = エラー
    fi
}
...
使い方: 各ペインで `claude` の代わりに `claude-task` で起動する。

| 背景色 | 意味 | 監督のアクション |
|--------|------|----------------|
| 青 | 実行中 | 放置。順調 |
| 緑 | 正常完了 | レビュー対象。次タスク投入 |
| 赤 | エラー終了 | 確認して対処 |
| デフォルト | 未起動 / リセット済み | タスク投入可能 |
...
# アクティビティのあるウィンドウをハイライト
set-window-option -g window-status-activity-style 'bg=colour160,fg=white'

# 手動で背景色を変えるキーバインド（巡回中に使う）
bind-key g select-pane -P 'bg=colour28'    # 緑
bind-key y select-pane -P 'bg=colour220'   # 黄
bind-key r select-pane -P 'bg=colour160'   # 赤
bind-key n select-pane -P 'bg=default'     # リセット
```
...
巡回時に一瞥するだけで、どのペインが完了/エラー/稼働中かわかる。

## 作業ディレクトリの原則

- 各Claudeは同じプロジェクトルートで起動する（Task Masterのタスクファイルを共有するため）
- 各Claudeが触るファイルスコープは分ける（衝突防止）
