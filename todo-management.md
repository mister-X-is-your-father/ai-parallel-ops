...
#### Task Studioのセットアップ（WSL2 + Tailscale環境）

WSL2ではinotifyが不安定なため、そのままではリアルタイム更新が動かない。セットアップスクリプトで以下を自動パッチする:

1. **chokidar polling有効化** — `ws.js`にusePolling追加（inotify回避）
2. **WebSocket URLのTailscale IP書き換え** — JSバンドル内の`ws://localhost`をTailscale IPに置換

```bash
# 手動セットアップ＆起動
./scripts/setup-task-studio.sh

# 停止
./scripts/setup-task-studio.sh --kill
```

`dotfiles/.bashrc` にて、tmuxセッション内で初回シェル起動時にこのスクリプトを自動実行する設定が入っている。
